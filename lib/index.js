...
function generate(userDetails, finalCharacters) {
	if (finalCharacters === null)
		finalCharacters = void 0;

	if (!validateUserDetails(userDetails))
		throw new Error('user details invalid');

	if (finalCharacters !== void 0 && !validateFinalChars(finalCharacters))
...
function validate(dln, userDetails) {
	...
	if (userDetails !== void 0 && !validateUserDetails(userDetails))
		throw new Error('user details invalid');
...
function generateSectionC(personalName) {
	const names = (personalName || '').split(/\s/).map(clean).filter(n => !!n);
	const initials = names.slice(0, 2).map(n => n.substr(0, 1)).join('');
